...
void live()
{
   ...
   // output sound
   tone(BUZZER_PIN,analogRead(NOTE_IN_PIN)+offsetFrequency,duration);
}
...
void updateTimer()
{
 ...
 //the amount of time passed is equal to current time minus start time
...
void updateArraysWithNoteAndTimings()
{
 ...
 if(countNotes < MAX_NOTES)
  {
     // need to add the average photocell change
     int testNote = analogRead(NOTE_IN_PIN);
    if(testNote>0)
    {
      notes[countNotes] = testNote; // possible vals?
      durations[countNotes] = timePassed; //makes a collection of durations longside the notes.
...
int getRunningAverage()
{
 ...
 runningAverageBuffer[nextCount] = activeFrequency;
 nextCount++;
 ...
 averageOffsetFreq = currentSum / RUNNING_SAMPLES;
 delay(100);  
...
{
  analogWrite(LED_PIN_R, 0);   // Turn on the LED -R
    //col is the intensity of the LED
...
void playWithDuration()
{
  ...
  for(int i=0; i<countNotes;i++)
  {
    ...
    /* https://www.arduino.cc/en/Reference/Tone */
    tone(BUZZER_PIN, notes[i], durations[i]); // plays buzzer with stored tone and duration
    delay(duration);
